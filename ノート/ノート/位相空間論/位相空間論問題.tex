...
\begin{prob}
  ...
  \begin{enumerate}
    ...
    \(X\)は可算コンパクト空間である。
    \item
    \(X\)のどんな可算無限部分集合も相対位相に関して離散集合でない。
  \end{enumerate}
\end{prob}
...
\begin{rem*}
  ...
  \(2^{2^{2^{\N}}}\)はLindel\"{o}fであってc.c.c.を満たすが可分ではない位相空間の例
  になっています (\autoref{Tychonoff}, \autoref{power separable}, \autoref{prod c.c.c.})。
\end{rem*}
...
\begin{prob}[積空間の可分性]\label{power separable}
  \(I\)を集合、\(i\)で添字付けられた二点集合の族\(2_i\dfn \{0,1\}\)を考える。
  各\(2_i\)には離散位相を入れて、位相空間とみなす。
  \(2^I\dfn \prod_{i\in \N}2_i\)を直積位相で位相空間とみなす。
  \begin{enumerate}
    \item
...
\begin{prob}[c.c.c.を満たす空間の積]\label{prod c.c.c.}
  \
  \begin{enumerate}
    \item
...
\begin{prob}[Urysohnの距離化定理]\label{Urysohn metrizable}
  ...
  \begin{enumerate}
    ...
    \item
    \(X\)は\textbf{第二可算}、つまり可算個の開集合からなる開基が存在すると仮定する。
    このとき\(X\)は可算な稠密部分集合を持つことを示しなさい。
    \item
    \(X\)は第二可算であるとする。
...
\begin{rem*}
  ...
  コンパクトハウスドルフ空間の圏から局所コンパクト空間の圏への忘却函手の左随伴函手である、
  ということを意味しています。
\end{rem*}
