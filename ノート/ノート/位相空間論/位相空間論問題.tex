...
\begin{prob}[非同相判定]
  ...
  \begin{enumerate}
    ...
    \item (超難).
    \(A \dfn [0,1]\times (0,1)\setminus (\Q\times \Q)\),
    \(X \dfn (\Q\cap (0,1)) \times \{0,1\}\),
    \(Y \dfn (\Q\cap (0,1)) \times \{0\}\),
    \(Z \dfn ([0,1]\setminus \Q) \times \{1\}\)とおく。
    \(A\cup X\)と
    \(A\cup Y\cup Z\)に\(\R^2\)の相対位相を入れたものは同相ではない。
    なぜでしょう。
    %点型完全コンパクト化の中で\(\Q\)的な部分の点の閉包の連結成分の数が違う。
  \end{enumerate}
\end{prob}
